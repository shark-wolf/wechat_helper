...
import pandas as pd
import pyperclip
from config import THEME, STEPS, PAGE_SIZE, SAFETY_CONFIG, ICON_PATH, load_coords, save_coord
...
class WeChatAddApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PC微信半自动辅助添加工具 (多选批量执行版)")
    ...
    def create_widgets(self):
        ...
        # 2. 表格与分页
        table_card = tk.Frame(self.root, bg=THEME["card_bg"], highlightbackground=THEME["border"], highlightthickness=1)
        table_card.pack(fill=tk.BOTH, expand=True, padx=15, pady=8)
        table_inner = tk.Frame(table_card, bg=THEME["card_bg"], padx=10, pady=10)
        table_inner.pack(fill=tk.BOTH, expand=True)

        columns = ("勾选", "序号", "手机号", "姓名", "状态", "操作")
        self.tree = ttk.Treeview(table_inner, columns=columns, show="headings", height=14)

        column_widths = {"勾选": 60, "序号": 60, "手机号": 150, "姓名": 130, "状态": 240, "操作": 120}
        for col, width in column_widths.items():
            self.tree.heading(col, text=col)
            self.tree.column(col, width=width, anchor="center")

        # 绑定点击事件：点击表头「勾选」切换全选；点击单行勾选框切换单选
        self.tree.heading("勾选", text="[  ] 全选", command=self.toggle_select_all)
        self.tree.bind("<ButtonRelease-1>", self.on_tree_cell_click)

        scrollbar = ttk.Scrollbar(table_inner, orient=tk.VERTICAL, command=self.on_scrollbar_scroll)
        ...
        page_card = tk.Frame(table_card, bg="#FAFAFA", highlightbackground=THEME["border"], highlightthickness=1)
    ...
    def open_calibration_dialog(self):
        ...
        tk.Label(
            dialog,
            text="选择标定项后，鼠标直接在微信对应窗口上点击目标按钮进行采集：",
            font=("Microsoft YaHei UI", 9, "bold"),
            bg=THEME["card_bg"],
            fg=THEME["text_main"]
        ).pack(anchor="w", padx=25, pady=(15, 6))

        lbl_hint = tk.Label(
            dialog,
            text="💡 当前状态: 未开启采集，请单选具体目标项",
        ...
        for val, label, key_name in opts:
            ...
            rb = tk.Radiobutton(
                ...
                fg=THEME["text_main"],
                command=lambda: on_radio_select()
            )
        ...
        ModernButton(btn_row, text="取消", command=on_cancel, bg="#E5E7EB", hover_bg="#D1D5DB", fg="#374151").pack(side=tk.RIGHT, padx=5)
    ...
    def import_excel_file(self):
        ...
        try:
            ...
            if "手机号" not in df.columns:
                messagebox.showerror("格式错误", "表格必须包含【手机号】列！")
                return
            ...
            self.log(f"✅ 成功载入数据: {os.path.basename(path)}，共 {len(self.df)} 条")
        ...
        if self.df is not None and not self.df.empty:
            ...
            for idx, row in self.df.iloc[start:end].iterrows():
                ...
                self.tree.insert("", tk.END, iid=idx, values=(
                    ...
                    idx + 1,
                    str(row["手机号"]).strip(),
                    str(row["姓名"]) if "姓名" in row and pd.notna(row["姓名"]) else "-",
                    str(row["状态"]) if pd.notna(row["状态"]) else "未添加",
                    ""
                ))
    ...
    def _run_batch_worker(self, target_indices):
        ...
        for i, idx in enumerate(target_indices):
            if self.cancel_requested or self.daily_added_count >= SAFETY_CONFIG["DAILY_MAX_LIMIT"]:
                break
            phone = str(self.df.at[idx, "手机号"]).strip()
    ...
    def _run_task_pipeline(self, idx: int) -> str:
        phone = str(self.df.at[idx, "手机号"]).strip()
        remark = str(self.df.at[idx, "姓名"]).strip() if "姓名" in self.df.columns and pd.notna(self.df.at[idx, "姓名"]) else ""
        ...
        result = self.bot.execute_add_pipeline(phone, remark)
